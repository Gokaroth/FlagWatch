...
import { fetchBuoyReadings, nearestReading } from "./nimh.mjs";
...
export async function buildAllBeachData({ fast = false } = {}) {
  ...
  // 1. Open-Meteo weather + marine, batched and in parallel, plus the NIMH buoy page.
  // The buoy feed is a single ~26 KB request for all six buoys, so it runs even in fast mode —
  // it is the only MEASURED data in the pipeline and is cheap enough to always include.
  const [weather, marine, buoys] = await Promise.all([
    fetchOpenMeteo(
      WEATHER_URL,
      latStr,
      lngStr,
  ...
  return beaches.map((beach, i) => {
    ...
    // Nearest real instrument, if one is close enough to mean anything. This NEVER feeds the
    // sea-state band — the band stays model-driven so all 56 beaches are computed the same way.
    // It is shown alongside, clearly labelled as measured, so a user can see the actual sea.
    const observed = nearestReading(beach.coordinates.lat, beach.coordinates.lng, buoys);
